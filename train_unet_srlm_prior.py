...
# load unet model
if not load_model:
    net = UNet(num_class = num_class, in_channels = 2)
    net = net.to(device)
else:
    net = UNet(num_class = num_class, in_channels = 2)
...
image_dataset = p.ImageDataset_withPrior(csv_file = c.train_val_csv)
...
## Prior flag - include a second input channel for UNet w/ prior
include_prior = True
"""
Comment this part if you don't need to regnerate patches
"""
...
#        print(sample['type'])
#        patches = p.GeneratePatches(sample, is_training = True, transform = True, multires =  False, prior = prior )
#    else:
#        print(sample['type'])
#        patches = p.GeneratePatches(sample, is_training = True, transform = False, multires =  False, prior = prior)
#        
